...
function AnimatedCounter({ target, label }) {
	...
	// When target changes (from SSE stream), gently animate to the new value
	useEffect(() => {
		if (count === target) return;

		let current = count;
		// Step should be fast, but relative to gap
...
const Landing = () => {
	...
	// Establish SSE connection for realtime stats
	useEffect(() => {
		const sse = new window.EventSource(API_ENDPOINTS.STATS.STREAM);
		...
		sse.addEventListener("error", (err) => {
			...
			sse.close();
			// Could implement reconnection logic if strictly necessary
		});
				...
				<div className="grid grid-cols-1 md:grid-cols-2 gap-8 max-w-5xl mx-auto">
					{features.map((feat, idx) => (
						<div
							...
							className={`relative ${feat.color} border-6 border-black p-8 transform transition-all duration-300 cursor-pointer ${hoveredCard === idx ? "scale-105" : ""
								}`}
							style={{
								boxShadow: "16px 16px 0px 0px rgba(0,0,0,1)",
								transform: `rotate(${feat.rotate}) ${hoveredCard === idx ? "scale(1.05)" : ""
									}`,
							}}
						>
			...
			<section className="bg-black text-white py-16 md:py-24 border-y-6 border-black">
				<div className="container mx-auto px-4">
					<div className="grid grid-cols-1 md:grid-cols-3 gap-8 justify-items-center text-center">
						<AnimatedCounter target={stats.users} label="Yappers" />
						<AnimatedCounter target={stats.messages} label="Messages" />
						<AnimatedCounter target={stats.groups} label="Groups" />
					</div>
				</div>
			</section>
				...
				<div className="container mx-auto px-4">
					...
					<div className="text-center border-t-2 border-white pt-6">
						<p className="font-bold">
							© YappHere. Made by - Shivam S, Knight-eGithub, Prasad, Sharan Lenwin Correa
